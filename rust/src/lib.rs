...
#[wasm_bindgen]
pub fn matrix_operations_i64(matrix_size: usize) -> i64 {
    let mut matrix1 = get_empty_matrix_i64(matrix_size);
    let mut matrix2 = get_empty_matrix_i64(matrix_size);
    fill_sequential_matrix_i64(&mut matrix1, matrix_size);
    fill_sequential_matrix_i64(&mut matrix2, matrix_size);
    
    let mut total_sum = 0;
    for i in 0..100 {
        let result = multiply_matrices_i64(&matrix1, &matrix2, i, matrix_size);
        total_sum += sum_of_matrix_elements_i64(&result, matrix_size);
    }
